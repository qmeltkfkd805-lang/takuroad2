...
--   RPC 들은 전부 `revoke execute on function ... from public, anon` 을 직접 쓰고 있고,
--   이 두 함수만 그게 빠져 있었다.
...
--       롤백에서도 복원하지 않는다. 취약점을 되살리는 롤백은 두지 않는다.
--   (2) search_path 를 'public' → 'pg_catalog','public','extensions','pg_temp' 로 바꾼다.
--       pg_temp 를 반드시 마지막에 둬서, 호출자가 임시 스키마에 같은 이름의 함수·테이블을
--       심어 definer 함수의 이름 해석을 가로채는 경로를 막는다.
--       함수 본문의 테이블은 전부 public. 으로 스키마를 명시했다.
--
--   owner(postgres), security definer(true),
--   postgres·service_role·authenticated 의 EXECUTE 는 기존과 동일하다.
--
--   ⚠️ 적용 전 확인: anon·authenticated 가 public 또는 extensions 스키마에
--      CREATE 권한을 가지면 pg_temp 를 마지막에 두는 것만으로는 부족하다.
--        select has_schema_privilege('anon','public','CREATE'),
--               has_schema_privilege('authenticated','public','CREATE'),
--               has_schema_privilege('anon','extensions','CREATE'),
--               has_schema_privilege('authenticated','extensions','CREATE');
--      네 값이 전부 false 여야 이 파일을 적용한다.
--
--   ⚠️ service_role 에 EXECUTE 를 주지만, service_role 로 호출하면 JWT 가 없어
--      auth.uid() 가 null 이라 관리자 게이트에서 42501 로 막힌다.
--      현재 서버 경로에서 이 함수들을 부르는 곳은 없다. 서버에서 써야 한다면
--      actor id 를 인자로 받는 별도 설계가 필요하다.
--
-- ============================================================
...
begin
  -- ── 관리자 게이트 ──
  -- 반드시 목록·total_count 를 계산하기 전에 막는다.
  -- 기존 함수에는 이 확인이 아예 없었다. anon 실행 권한과 합쳐져 회원 목록이 공개돼 있었다.
  -- 클라이언트가 보내는 어떤 값도 신뢰하지 않는다. 역할은 DB 에서 직접 읽는다.
  ...
  -- 페이지 인자 검증. null·음수·과대값을 그대로 넘기지 않는다.
  -- 상한 100 은 한 번의 호출로 전체 회원을 덤프하지 못하게 하는 안전장치다.
  if page_limit is null or page_limit < 1 or page_limit > 100 then
    raise exception 'page_limit 은 1~100 이어야 합니다: %', page_limit using errcode = '22023';
  end if;
  if page_offset is null or page_offset < 0 then
    raise exception 'page_offset 은 0 이상이어야 합니다: %', page_offset using errcode = '22023';
  end if;
...
revoke execute on function public.get_admin_members(text, int, int, text, text, text)
  from public, anon;
...
revoke execute on function public.admin_set_member_role(uuid, text) from public, anon;
grant execute on function public.admin_set_member_role(uuid, text) to authenticated, service_role;
...
--
-- 기대:
--   1. 3개 모두 owner=postgres / definer=true /
...
--
-- 동명 오버로드가 남지 않았는지도 확인한다:
-- select p.proname::text,
--        pg_get_function_identity_arguments(p.oid)::text as 인자,
--        count(*) over (partition by p.proname) as 같은이름_개수
-- from pg_proc p
-- where p.pronamespace='public'::regnamespace
--   and p.proname in ('get_admin_members','admin_set_member_role','admin_grant_exp')
-- order by 1;
--
-- 기대: 세 이름 각각 1개씩만.
--   get_admin_members 의 인자는 정확히
--   'text, integer, integer, text, text, text' 여야 한다.
...
-- [롤백]
-- ============================================================
-- -- get_admin_members 를 원래 시그니처(3인자)로 되돌린다.
-- -- ⚠️ 보안 요소 3가지는 되돌리지 않는다. 취약점을 되살리는 롤백은 두지 않는다.
-- --    (a) 관리자 게이트 유지  (b) 강화된 search_path 유지
-- --    (c) anon·PUBLIC EXECUTE 복원하지 않음
...
--                   where p.id = v_actor and p.role = 'admin') then
--     raise exception '관리자만 조회할 수 있습니다' using errcode = '42501';
--   end if;
--   return query
--   select p.id, p.nickname, p.avatar_url, p.role, p.created_at,
...
-- end;
-- $$;
-- revoke execute on function public.get_admin_members(text, int, int) from public, anon;
-- grant execute on function public.get_admin_members(text, int, int)
--   to authenticated, service_role;
--
-- drop function if exists public.admin_set_member_role(uuid, text);
--
-- -- admin_grant_exp 를 검증 없던 버전으로 (권장하지 않는다 — 음수 지급이 다시 열린다)
-- -- search_path 와 실행 권한은 되돌리지 않는다.
-- create or replace function public.admin_grant_exp(uid uuid, amount integer, reason text)
-- returns json
-- language plpgsql
-- security definer
