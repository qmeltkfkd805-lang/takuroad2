...
const PROVIDE = [
  { icon: '🎯', title: '애니메이션 팬 타겟', desc: '덕질에 진심인 사용자에게 정확히 닿아요.' },
  { icon: '🛍️', title: '굿즈 관심 사용자', desc: '피규어·굿즈를 실제로 사러 다니는 사람들이에요.' },
  { icon: '🗺️', title: '지도 기반 노출', desc: '매장을 지도에서 발견하고 직접 찾아가요.' },
  { icon: '📚', title: '작품 기반 탐색', desc: '좋아하는 작품으로 매장·이벤트를 탐색해요.' },
]

const TYPES = [
  { icon: '🏪', title: '굿즈샵 등록·인증', desc: '매장을 공식 등록하고 사장님 인증을 받아요.' },
  { icon: '🎉', title: '이벤트·팝업 홍보', desc: '팝업스토어·전시·행사를 널리 알려요.' },
  { icon: '🤝', title: '브랜드·기업 제휴', desc: '브랜드 협업과 공동 기획을 함께해요.' },
  { icon: '📣', title: '광고·프로모션', desc: '배너·추천 노출로 더 많은 사람에게 닿아요.' },
  { icon: '✨', title: '콘텐츠 협업', desc: '함께 만드는 기획 콘텐츠와 캠페인.' },
  { icon: '💬', title: '기타 제안', desc: '어떤 형태든 좋아요. 편하게 제안해 주세요.' },
]
        ...
        <div className={styles.provideGrid}>
          {PROVIDE.map(p => (
            <div key={p.title} className={styles.provideCard}>
              <span className={styles.provideIcon}>{p.icon}</span>
