...
import { getReportedFanArts, getFanArtAppeals, restoreFanArt, deleteFanArt } from '@/services/fanArtService'
import { ReportedFanArt, FanArtAppeal, REASON_LABEL } from '@/types/fan-art'

export default function FanArtReportsTab() {
  const [items, setItems] = useState<ReportedFanArt[]>([])
  const [loading, setLoading] = useState(true)

  const load = useCallback(async () => {
    setLoading(true)
    const list = await getReportedFanArts()
  ...
  const onRestore = async (id: string) => {
    if (!window.confirm('이 팬아트를 다시 공개할까요?')) return
    await restoreFanArt(id); load()
  }
  const onDelete = async (id: string) => {
    if (!window.confirm('이 팬아트를 삭제할까요? 되돌릴 수 없어요.')) return
    await deleteFanArt(id); load()
  }

  if (loading) return <p style={{ color: 'var(--muted)', padding: 20 }}>불러오는 중…</p>
  if (items.length === 0) return <p style={{ color: 'var(--muted)', padding: 20 }}>신고되거나 숨김 처리된 팬아트가 없어요.</p>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '4px 0' }}>
      {items.map(item => (
        <ReportCard key={item.art.id} item={item} onRestore={onRestore} onDelete={onDelete} />
      ))}
    </div>
  )
}

function ReportCard({ item, onRestore, onDelete }: { item: ReportedFanArt; onRestore: (id: string) => void; onDelete: (id: string) => void }) {
  const { art, reportCount, reasonCounts, reports } = item
  const hidden = art.status === 'hidden'
  const [appeals, setAppeals] = useState<FanArtAppeal[] | null>(null)
  const [showAppeals, setShowAppeals] = useState(false)

  const loadAppeals = async () => {
    if (appeals === null) setAppeals(await getFanArtAppeals(art.id))
    setShowAppeals(v => !v)
  }
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 14, background: 'var(--surface)', overflow: 'hidden' }}>
      <div style={{ display: 'flex', gap: 14, padding: 14 }}>
        <img src={art.imageUrl} alt="" style={{ width: 88, height: 88, flexShrink: 0, borderRadius: 10, objectFit: 'cover', background: 'var(--surface2)' }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: hidden ? '#c0392b' : 'var(--muted)', background: hidden ? 'rgba(239,90,90,.1)' : 'var(--surface2)', padding: '2px 8px', borderRadius: 9999 }}>
              {hidden ? `임시 숨김 (${art.hiddenBy === 'auto' ? '자동' : '관리자'})` : '공개중'}
            </span>
            <span style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--accent)' }}>신고 {reportCount}건</span>
          </div>
          <div style={{ fontSize: 12.5, color: 'var(--muted)', marginBottom: 8 }}>{art.work?.name ?? ''} · {art.author?.nickname ?? '익명'}</div>
      ...
      <div style={{ borderTop: '1px solid var(--border)', padding: '12px 14px', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        {hidden && (
          <button onClick={() => onRestore(art.id)} style={{ padding: '9px 16px', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontWeight: 800, fontSize: 13.5, cursor: 'pointer', fontFamily: 'inherit' }}>다시 공개</button>
        )}
        <button onClick={() => onDelete(art.id)} style={{ padding: '9px 16px', borderRadius: 10, border: 'none', background: '#e04343', color: '#fff', fontWeight: 800, fontSize: 13.5, cursor: 'pointer', fontFamily: 'inherit' }}>삭제</button>
      </div>
    </div>
  )
}
