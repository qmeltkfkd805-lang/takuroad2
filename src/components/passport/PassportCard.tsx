...
import { uploadAvatar } from '@/services/shopService'
...
export default function PassportCard({ passport, isOwner, onCustomizeClick }: Props) {
  ...
  async function onPickAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return
    setUploading(true)
    const res = await uploadAvatar(passport.userId, file)
    setUploading(false)
    if (res.ok && res.url) setAvatarUrl(res.url)
    else alert(res.error ?? '업로드에 실패했어요')
  }
    ...
    >
      ...
      <div className={styles.face}>
        <div className={styles.avatarWrap}>
          ...
          {isOwner && (
            <>
              <button className={styles.avatarEdit} onClick={() => fileRef.current?.click()} disabled={uploading}>
