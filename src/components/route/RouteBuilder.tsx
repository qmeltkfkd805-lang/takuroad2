...
export default function RouteBuilder() {
  ...
  const { user } = useAuth()
  const editRouteId: string | null = null
  const editing = false
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', border: '1px solid var(--border)', borderRadius: 14, padding: 16, marginBottom: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <h3 style={{ fontSize: 16, fontWeight: 900, margin: 0 }}>루트 만들기</h3>
        <button onClick={() => router.back()} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 14, fontFamily: 'inherit' }}>취소 ✕</button>
      </div>
