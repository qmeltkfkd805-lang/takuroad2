import { MemorialKind, MemorialData, ROUTE_TYPE_CATEGORY } from './types'
...
export function varsFromData(data: MemorialData): MessageVars {
  return {
    ...
    category: data.routeType ? ROUTE_TYPE_CATEGORY[data.routeType] : undefined,
  }
}
...
function fillVars(template: string, vars: MessageVars): string | null {
  ...
  return template
    ...
    .replace(/\{title\}/g, String(vars.title ?? ''))
