...
if (process.env.ALLOW_LEGACY_PATH_IMPORT !== 'I-UNDERSTAND-THIS-RECREATES-DUPLICATES') {
  ...
  console.error('  script : registerConan30PurchaseBenefits.mjs')
...
const rows = Array.from({ length: 4 }, (_, index) => ({
  name: `1·2차 MD 구매 특전 안내 ${index + 1}`,
  filename: `purchase-benefit-${index}.jpg`,
}))
...
const backup = `scripts/event-goods-backups/before-conan30-benefits-${stamp}.json`
