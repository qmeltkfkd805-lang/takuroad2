...
if (process.env.ALLOW_LEGACY_PATH_IMPORT !== 'I-UNDERSTAND-THIS-RECREATES-DUPLICATES') {
  ...
  console.error('  script : registerDungeonMeshiWeek10Benefits.mjs')
...
const eventId = '9fbd3888-7011-4e9e-8d65-3919aa682f51'
const name = '10주차 입장·구매 특전'

const { data: before, error: beforeError } = await db.from('event_goods').select('*').eq('event_id', eventId)
if (beforeError) throw beforeError
await mkdir('scripts/event-goods-backups', { recursive: true })
const stamp = new Date().toISOString().replaceAll(':', '-').replaceAll('.', '-')
const backup = `scripts/event-goods-backups/before-dungeon-meshi-week10-benefits-${stamp}.json`
...
const objectPath = `${eventId}/official-x-week10-entry-purchase-benefits.jpg`
const image = await readFile('scripts/work-menu-goods-images/dungeon-meshi/benefit-2.jpg')
