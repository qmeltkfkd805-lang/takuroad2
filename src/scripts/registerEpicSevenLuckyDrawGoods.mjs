...
if (process.env.ALLOW_LEGACY_PATH_IMPORT !== 'I-UNDERSTAND-THIS-RECREATES-DUPLICATES') {
  ...
  console.error('  script : registerEpicSevenLuckyDrawGoods.mjs')
...
const sourceUrl = 'https://x.com/smgcafe_kr/status/2092477822053011674'
...
const eventBackup = `scripts/event-goods-backups/before-epic-seven-luckydraw-event-${stamp}.json`
const goodsBackup = `scripts/event-goods-backups/before-epic-seven-luckydraw-goods-${stamp}.json`
...
const rows = [
  { name: '럭키드로우 포토카드 MD 프리뷰', file: '01.jpg' },
  { name: '럭키드로우 포토카드 캐릭터 라인업 1', file: '02.jpg' },
  { name: '럭키드로우 포토카드 캐릭터 라인업 2', file: '03.jpg' },
  { name: '럭키드로우 포토카드 뒷면·구성 안내', file: '04.jpg' },
]
...
for (const row of rows) {
  ...
  const objectPath = `${eventId}/official-x-luckydraw-${row.file}`
  const image = await readFile(`scripts/work-menu-goods-images/epic7-luckydraw/${row.file}`)
