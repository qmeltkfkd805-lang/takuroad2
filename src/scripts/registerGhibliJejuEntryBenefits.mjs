...
if (process.env.ALLOW_LEGACY_PATH_IMPORT !== 'I-UNDERSTAND-THIS-RECREATES-DUPLICATES') {
  ...
  console.error('  script : registerGhibliJejuEntryBenefits.mjs')
...
const eventId = 'fb6b3be6-728c-4281-bf5b-c24544994d96'
const name = '입장 특전 클리어파일·원형 부채'

const { data: before, error: beforeError } = await db.from('event_goods').select('*').eq('event_id', eventId)
if (beforeError) throw beforeError
await mkdir('scripts/event-goods-backups', { recursive: true })
const stamp = new Date().toISOString().replaceAll(':', '-').replaceAll('.', '-')
const backup = `scripts/event-goods-backups/before-ghibli-jeju-entry-benefits-${stamp}.json`
...
const objectPath = `${eventId}/official-popcond-entry-benefits.jpg`
const image = await readFile('scripts/work-menu-goods-images/ghibli-jeju/img-4.jpg')
