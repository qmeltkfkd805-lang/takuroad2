...
if (process.env.ALLOW_LEGACY_PATH_IMPORT !== 'I-UNDERSTAND-THIS-RECREATES-DUPLICATES') {
  ...
  console.error('  script : registerIchibankujiMusinsaBenefit.mjs')
...
const eventId = '90ab054c-20a2-4a0b-9ba3-13b00c09ceb8'
const name = '3회 이상 구매 특전 이치방쿠지 스트링백'

const { data: before, error: beforeError } = await db.from('event_goods').select('*').eq('event_id', eventId)
if (beforeError) throw beforeError
await mkdir('scripts/event-goods-backups', { recursive: true })
const stamp = new Date().toISOString().replaceAll(':', '-').replaceAll('.', '-')
const backup = `scripts/event-goods-backups/before-ichibankuji-musinsa-benefit-${stamp}.json`
...
const objectPath = `${eventId}/official-instagram-string-bag-benefit.jpg`
const image = await readFile('scripts/work-menu-goods-images/ichibankuji-musinsa/official-post.jpg')
