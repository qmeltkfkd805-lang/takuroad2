...
if (process.env.ALLOW_LEGACY_PATH_IMPORT !== 'I-UNDERSTAND-THIS-RECREATES-DUPLICATES') {
  ...
  console.error('  script : registerReborn20MenuGoods.mjs')
...
const rows = [
  ...Array.from({ length: 9 }, (_, index) => ({ name: `오리지널 공식 MD 미리보기 ${index + 1}`, filename: `original-${index}.jpg` })),
  ...Array.from({ length: 3 }, (_, index) => ({ name: `일본 공식 수입 굿즈 목록 ${index + 1}`, filename: `imported-${index}.jpg` })),
  { name: '1주차 구매 특전 안내', filename: 'benefit-0.jpg' },
]
...
const backup = `scripts/event-goods-backups/before-reborn20-${stamp}.json`
...
for (const row of rows) {
  ...
  const { error: insertError } = await db.from('event_goods').insert({
    event_id: eventId, name: row.name, kind: 'goods', price: null, image_url: publicData.publicUrl,
