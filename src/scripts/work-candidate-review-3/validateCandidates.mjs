...
const candidateValues = candidates.flatMap((candidate) => [candidate.name, candidate.englishName, ...(candidate.aliases ?? [])].filter(Boolean).map((value) => norm(value)))

const result = {
  batches: batchFiles.length,
  reviewed: candidates.length,
  existingRows: existing.length,
  liveConflicts: [...new Set(liveConflicts)],
  candidateValueDuplicates: duplicateValues(candidateValues),
