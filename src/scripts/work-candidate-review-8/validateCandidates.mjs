...
for (let from = 0; ; from += 1000) {
  const { data, error } = await supabase.from('tags').select('name, slug, english_name, aliases').order('name').range(from, from + 999)
  if (error) throw error
  existing.push(...data)
  if (data.length < 1000) break
}
const allowedGenres = new Set(['액션', '격투', '판타지', '모험', '학원', '일상', '가족', 'SF', '추리', '일상', '로맨스', 'BL', 'GL', '코미디', '스포츠', '음악', '아이돌', '요리', '호러', '드라마', '마법소녀', '소년물', '로봇/메카', '19', '고어', '기타'])
const allowedTypes = new Set(['웹툰', '웹소설', '소설', '애니', '영화', '웹드라마', '만화', '버튜버', '캐릭터', '게임', '카드게임', '완구', '보컬로이드', '브랜드', '제작사'])
...
if (existing.length !== 1039 || result.liveConflicts.length || result.candidateValueDuplicates.length || result.candidateSlugDuplicates.length || result.badGenres.length || result.badTypes.length || result.over80.length) process.exitCode = 1
